export default async function handler(req, res) {
  ...
  if (!message || message.trim().length < 5) {
    return res.status(400).json({ urgency: 'aucune' });
  }
...
- "suicide" : idéations suicidaires claires, "j'en peux plus de vivre", "envie d'en finir", "je veux mourir", auto-destruction explicite
...
- "j'en ai marre" SEUL → "aucune" (expression banale)
...
- mention de violence subie passée pour réflexion → "aucune" (ne pas déclencher en boucle)
...
Message à analyser : "${message.trim()}"`;
